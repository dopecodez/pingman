...
const normalize = (val: any) => ({
  ...val,
  bufferSize: val.bufferSize ?? undefined,
});
...
test("ouput for windows IPV6 ping", async (t) => {
  ...
  t.deepEqual(output, normalize(response.windows_ipv6));
});
...
test("ouput for linux IPV4 ping", async (t) => {
  ...
  t.deepEqual(output as any, normalize(response.linux_ipv4));
});
...
test("ouput for linux IPV6 ping", async (t) => {
  ...
  t.deepEqual(output as any, normalize(response.linux_ipv6));
});
...
test("ouput for mac IPV4 ping", async (t) => {
  ...
  t.deepEqual(output as any, normalize(response.macos_ipv4));
});
